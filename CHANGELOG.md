...
- **Backend contract sync (2026-07-16)** — refonte alignée sur les endpoints réels de `skilluv-backend`, discovered post-MVP. All 12 previously-listed backend gaps are now resolved on the server side; the frontend was re-shaped 1-for-1:
  ...
  - **`PayoutRequestModal`** — reworked to switch between EUR/Stripe and XOF/Momo, with method-specific readiness gates (`stripe_kyc_status === 'verified'` / `momo_phone_verified`), country field for Stripe onboarding, provider radio for Momo (orange / mtn / wave), amount validation against the per-method available balance.
  - **`privacyApi` refonte** — 4 méthodes contre 6 avant. `getConsents` supprimé (l'état vit sur `users.*` et remonte via `/auth/me`). `patchConsents` → `recordConsent` sur `POST /legal/consent`. `requestGdprExport` + `requestDataExport` fusionnés en `requestDataExport` sur `POST /auth/me/data-export`. `requestAccountDeletion` → `deleteAccount` sur `DELETE /auth/account`. Ajout de `consentVersion` pour lire les URLs légales officielles.
  - **Page `/settings/privacy`** — les toggles ré-envoient le duo `{analytics, marketing}` complet à chaque changement (endpoint unique).
  - **`moderationApi.forum` refonte** — actions du contrat backend: `hide|unhide|lock|unlock` (les précédentes `delete|mute_author|mark_spam` étaient fictives). `muteAuthor` renommé `muteUser`, accepte un `scope` (`forum|community|all`), retourne `{mute_id, expires_at}`. Le forum post détail présente un menu dynamique qui expose Hide/Unhide et Lock/Unlock selon l'état actuel du post.
  - **`moderationApi.community.rejectChallenge`** — clé du body renommée `reason` → `feedback` (contrat backend, min 8 caractères validés serveur). `approveChallenge` ne prend plus de body.
  - **`moderationApi.community.reviewQueue`** — nouveau (remplace `communityApi.pendingReview`, retiré). Le type retour est `CommunityReviewItem` (id, title, description, review_feedback, created_by, created_at) et non un `Challenge` complet.
  - **`FlaggedDeliverable`** — shape aligné backend: `{id, user_id, verification_status, verification_signal, submitted_at}`. La page `/moderation/plagiarism` extrait le `plagiarism_score` et `similar_to` depuis `verification_signal` au lieu de champs plats fictifs.
  - **Types** — nouveaux `Wallet`, `StripeKycStatus`, `ForumModerateAction`, `MuteScope` alignés Rust. `WalletBalance`, `PayoutRequest`, l'ancien shape polymorphique de moderation retirés du barrel.
  - **`ForumPost.deleted_at`** — champ ajouté au type frontend (backend le sert quand un modérateur masque un post via `hide`).
  - **Docs** — `BACKEND-GAPS.md` transformé en journal de closure (statut: tous les endpoints attendus livrés, contrats consommés listés, ce que le sync a corrigé). Aucun endpoint à créer côté backend.
  - **Tests** — 3 fichiers réécrits contre les nouveaux contrats (`wallet-flows.test.ts`, `moderation.test.ts`, sections `fe-m10.test.ts` privacy + `api-p16-p25.test.ts` wallet/moderation). **119/119 tests verts.**
