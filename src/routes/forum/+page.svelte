	...
	<aside>
		...
		<div class="space-y-1">
			<button
				...
				class="w-full text-left rounded-lg px-3 py-2 text-sm font-medium transition-colors {selectedCategory ===
				'all'
		...
		<div class="space-y-1">
			{#each [{ v: 'recent' as const, fr: 'Récent', en: 'Recent' }, { v: 'hot' as const, fr: 'Populaires', en: 'Hot' }, { v: 'top-bounty' as const, fr: 'Top bounty', en: 'Top bounty' }] as s}
				<button
					onclick={() => {
						sort = s.v;
						void load();
					}}
					class="w-full text-left rounded-lg px-3 py-2 text-sm font-medium transition-colors {sort ===
					s.v
						? 'bg-primary/15 text-primary'
						: 'hover:bg-surface-overlay'}"
				>
	...
	<main>
		...
		{#if loading}
			<div class="space-y-2">
				{#each Array(6) as _}
					<div
						class="animate-pulse rounded-2xl border border-border bg-surface-elevated h-24"
					></div>
				{/each}
			</div>
					...
					>
						<div class="flex items-start gap-3">
							...
							<div class="flex flex-col items-center text-center shrink-0 min-w-[52px]">
								{#if p.bounty_fragments > 0}
									<div class="mb-1">
										<span class="text-[10px] font-bold uppercase tracking-wider text-accent"
											>Bounty</span
										>
										<div class="text-2xl font-black text-accent leading-none">
											{p.bounty_fragments}
										</div>
									</div>
								{:else}
								...
								{/if}
								<div
									class="mt-2 text-sm font-bold {p.comment_count > 0
										? 'text-primary'
										: 'text-text-muted'}"
								>
									{p.comment_count}
							...
							<div class="min-w-0 flex-1">
								<div class="mb-1 flex items-center gap-2 flex-wrap">
									...
									{#if p.accepted_answer_id}<Badge variant="success" size="sm"
											>✓ {i18n.locale === 'fr' ? 'Résolu' : 'Solved'}</Badge
										>{/if}
								</div>
								<h2
									class="text-lg font-semibold leading-snug line-clamp-2 hover:text-primary transition-colors"
								>
									{p.title}
								</h2>
