...
<svelte:head>
	...
	<meta
		property="og:title"
		content="Skilluv | {i18n.t('landing.title')} {i18n.t('landing.titleAccent')}"
	/>
...
{#if auth.isAuthenticated}
	...
	<div class="mx-auto max-w-5xl px-4 py-8">
		<h1 class="mb-2 text-3xl font-bold">
			{@html i18n.t('dashboard.greeting', {
				name: `<span class="text-accent">${escapeHtml(auth.displayName)}</span>`
			})}
		</h1>
		<p class="mb-8 text-text-muted">
			...
			{/if}
			· {auth.user?.total_fragments ?? 0}
			{i18n.t('common.fragments')} · {i18n.t('common.streak')}
			{auth.user?.streak_current ?? 0}j
		</p>
		...
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
			...
			>
				<div
					class="flex h-12 w-12 items-center justify-center rounded-xl bg-accent/10 text-accent mb-4 transition-transform duration-300 group-hover:scale-110"
				>
					<svg
						class="h-6 w-6"
						fill="none"
				...
				<h2 class="mb-2 text-lg font-semibold group-hover:text-accent transition-colors">
					{i18n.t('dashboard.cardChallenges')}
				</h2>
			...
			>
				<div
					class="flex h-12 w-12 items-center justify-center rounded-xl bg-primary/10 text-primary mb-4 transition-transform duration-300 group-hover:scale-110"
				>
					<svg
						class="h-6 w-6"
						fill="none"
				...
				<h2 class="mb-2 text-lg font-semibold group-hover:text-primary transition-colors">
					{i18n.t('dashboard.cardProfile')}
				</h2>
			...
			>
				<div
					class="flex h-12 w-12 items-center justify-center rounded-xl bg-warning/10 text-warning mb-4 transition-transform duration-300 group-hover:scale-110"
				>
					<svg
						class="h-6 w-6"
						fill="none"
				...
				<h2 class="mb-2 text-lg font-semibold group-hover:text-warning transition-colors">
					{i18n.t('dashboard.cardLeaderboards')}
				</h2>
		...
		<div class="mt-8 grid gap-3 sm:grid-cols-4">
			{#each [{ href: '/feed', icon: '◎', fr: 'Fil', en: 'Feed' }, { href: '/forum', icon: '◈', fr: 'Forum', en: 'Forum' }, { href: '/guilds', icon: '⬢', fr: 'Guildes', en: 'Guilds' }, { href: '/messages', icon: '◎', fr: 'Messages', en: 'Messages' }] as short}
				<a
					href={short.href}
					class="flex items-center gap-3 rounded-xl border border-border bg-surface-elevated p-4 hover:border-primary/40 hover:-translate-y-0.5 transition-all"
				>
					<div
