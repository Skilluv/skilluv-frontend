<script lang="ts">
	...
	let ctaHref = $derived(
		auth.isAuthenticated ? '/enterprise/bounties/new' : '/enterprise/register?intent=bounty'
	);
	let ctaLabel = $derived(
		auth.isAuthenticated
			? i18n.locale === 'fr'
				? 'Poster ma première bounty'
				: 'Post my first bounty'
			: i18n.locale === 'fr'
...
<svelte:head>
	<title
		>{i18n.locale === 'fr'
			? 'Bounties open-source pour entreprises | Skilluv'
			: 'Open-source bounties for companies | Skilluv'}</title
	>
	<meta
		name="description"
...
<section class="py-16 sm:py-24 lg:py-32">
	<div class="mx-auto max-w-6xl px-4">
		...
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
			{#each [{ n: '01', fr: { t: 'Vitesse', d: 'Un dev intéressé peut revendiquer en 5 minutes. Pas de recrutement, pas de screening, pas de contrat.' }, en: { t: 'Speed', d: 'An interested dev can claim in 5 minutes. No recruitment, no screening, no contract.' } }, { n: '02', fr: { t: 'Coût variable', d: "Vous payez uniquement au merge de la PR. Aucune facture d'attente, aucun engagement mensuel." }, en: { t: 'Variable cost', d: 'You pay only when the PR merges. No pending invoice, no monthly commitment.' } }, { n: '03', fr: { t: 'Qualité prouvée', d: "Chaque talent affiche son historique de challenges résolus, sa progression, ses badges. Vous savez à qui vous confiez l'issue." }, en: { t: 'Proven quality', d: 'Each talent shows their history of solved challenges, progression, badges. You know who you trust the issue to.' } }, { n: '04', fr: { t: 'Zéro RH', d: "Pas d'onboarding, pas d'accès à donner, pas de review interne. Le code arrive via une PR standard sur GitHub." }, en: { t: 'Zero HR', d: 'No onboarding, no access to grant, no internal review. Code arrives via a standard GitHub PR.' } }] as b}
...
<section class="border-t border-border bg-surface-elevated/40 py-16 sm:py-24 lg:py-32">
	<div class="mx-auto max-w-6xl px-4">
		...
		<div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
			{#each [{ n: '01', fr: { t: 'Vous postez', d: "URL GitHub de l'issue, contexte, compétences requises, récompense. Les crédits sont débités et séquestrés immédiatement." }, en: { t: 'You post', d: 'GitHub issue URL, context, required skills, reward. Credits are debited and escrowed immediately.' } }, { n: '02', fr: { t: 'Un talent revendique', d: 'Un talent qualifié la revendique. Il a 7 jours pour ouvrir une PR sur votre repo.' }, en: { t: 'A talent claims', d: 'A qualified talent claims it. They have 7 days to open a PR on your repo.' } }, { n: '03', fr: { t: 'Vous mergez', d: "Vous reviewez la PR comme n'importe quelle contribution. Merge, c'est vous qui décidez." }, en: { t: 'You merge', d: 'You review the PR like any other contribution. Merge is your call.' } }, { n: '04', fr: { t: 'Payout automatique', d: 'Le webhook GitHub détecte le merge. Les crédits séquestrés sont convertis en fragments pour le talent, la bounty est marquée payée.' }, en: { t: 'Automatic payout', d: 'The GitHub webhook detects the merge. Escrowed credits are converted into fragments for the talent, bounty is marked paid.' } }] as step}
	...
	<div class="mx-auto max-w-6xl px-4">
		<div class="mb-10 sm:mb-16 max-w-3xl">
			<h2 class="text-4xl sm:text-5xl lg:text-6xl font-black leading-[1.05] tracking-tight mb-5">
				...
				<span class="text-accent"
					>{i18n.locale === 'fr' ? 'vos autres options.' : 'your other options.'}</span
				>
			</h2>
		</div>
		...
		<div class="grid gap-4 sm:grid-cols-3">
			...
			<div class="rounded-2xl border border-border bg-surface-elevated overflow-hidden">
				...
				<div class="p-5 space-y-2 text-sm">
					<p class="flex gap-2 text-text-muted">
						...
						{i18n.locale === 'fr'
							? 'Recherche, screening, entretien'
							: 'Search, screening, interview'}
					</p>
					<p class="flex gap-2 text-text-muted">
						<span class="text-text-muted">–</span>
						{i18n.locale === 'fr' ? 'Contrat, factures, TVA' : 'Contract, invoices, VAT'}
					</p>
					<p class="flex gap-2 text-text-muted">
						<span class="text-text-muted">–</span>
						{i18n.locale === 'fr' ? 'Onboarding accès repo' : 'Repo access onboarding'}
					</p>
					<p class="flex gap-2 text-text-muted">
						<span class="text-text-muted">–</span>
						{i18n.locale === 'fr'
							? 'Vous payez même si le travail ne merge pas'
							: "You pay even if the work doesn't merge"}
					</p>
				</div>
			</div>
			...
			<div class="rounded-2xl border border-border bg-surface-elevated overflow-hidden">
				<div class="flex items-center gap-3 border-b border-border px-5 py-3">
					...
					<span class="text-sm font-semibold text-text-muted"
						>{i18n.locale === 'fr' ? 'Embauche' : 'Hiring'}</span
					>
				</div>
				<div class="p-5 space-y-2 text-sm">
					<p class="flex gap-2 text-text-muted">
						<span class="text-text-muted">–</span>
						{i18n.locale === 'fr' ? 'Cycle de 3-6 mois' : '3-6 month cycle'}
					</p>
					<p class="flex gap-2 text-text-muted">
						<span class="text-text-muted">–</span>
						{i18n.locale === 'fr'
							? 'Charges sociales, salaire fixe'
							: 'Payroll taxes, fixed salary'}
					</p>
					<p class="flex gap-2 text-text-muted">
						<span class="text-text-muted">–</span>
						{i18n.locale === 'fr' ? 'Overhead RH permanent' : 'Permanent HR overhead'}
					</p>
					<p class="flex gap-2 text-text-muted">
						<span class="text-text-muted">–</span>
						{i18n.locale === 'fr'
							? 'Sur-dimensionné pour une issue isolée'
							: 'Overkill for an isolated issue'}
					</p>
				</div>
			</div>
			...
			<div class="rounded-2xl border border-border bg-surface-elevated overflow-hidden">
				...
				<div class="p-5 space-y-2 text-sm">
					<p class="flex gap-2 text-text-primary">
						<span class="text-success font-bold">+</span>
						{i18n.locale === 'fr' ? 'Publication en 3 minutes' : 'Publish in 3 minutes'}
					</p>
					<p class="flex gap-2 text-text-primary">
						<span class="text-success font-bold">+</span>
						{i18n.locale === 'fr'
							? 'Payout au merge, séquestre garanti'
							: 'Payout on merge, guaranteed escrow'}
					</p>
					<p class="flex gap-2 text-text-primary">
						<span class="text-success font-bold">+</span>
						{i18n.locale === 'fr'
							? 'Aucun accès repo à donner (PR standard)'
							: 'No repo access needed (standard PR)'}
					</p>
					<p class="flex gap-2 text-text-primary">
						<span class="text-success font-bold">+</span>
						{i18n.locale === 'fr'
							? 'Profil talent prouvé (challenges publics)'
							: 'Proven talent profile (public challenges)'}
					</p>
				</div>
			</div>
		</div>
		...
		<div class="grid gap-4 sm:grid-cols-3">
			<div class="rounded-2xl border border-border bg-surface-elevated overflow-hidden">
				...
				<div class="p-5">
					...
					<p class="font-mono text-xs text-accent">
						{i18n.locale === 'fr'
							? 'ex : Rails 6 → 7 · Node 16 → 20 · Vue 2 → 3'
							: 'ex: Rails 6 → 7 · Node 16 → 20 · Vue 2 → 3'}
					</p>
				</div>
			</div>
			...
			<div class="rounded-2xl border border-border bg-surface-elevated overflow-hidden">
				...
				<div class="p-5">
					...
					<p class="font-mono text-xs text-accent">
						{i18n.locale === 'fr'
							? 'ex : race condition async · fuite mémoire · rendu Safari cassé'
							: 'ex: async race condition · memory leak · broken Safari render'}
					</p>
				</div>
			</div>
			...
			<div class="rounded-2xl border border-border bg-surface-elevated overflow-hidden">
				...
				<div class="p-5">
					...
					<p class="font-mono text-xs text-accent">
						{i18n.locale === 'fr'
							? 'ex : SSO SAML · dark mode · widget embed · API pagination cursor'
							: 'ex: SAML SSO · dark mode · embed widget · cursor pagination API'}
					</p>
				</div>
			</div>
		</div>
...
<FaqSection
	items={i18n.locale === 'fr'
		? [
				{
					q: 'Et si personne ne merge la PR ?',
					a: "Les crédits séquestrés vous sont remboursés à l'expiration de la bounty (30 jours par défaut, configurable). Vous ne payez que si la PR est mergée par votre équipe."
				},
				{
					q: 'La qualité du code est-elle garantie ?',
					a: "La qualité est validée par votre review — vous êtes libre de rejeter la PR. Le talent est incité à livrer propre car son profil publique enregistre le taux d'acceptation."
				},
				{
					q: 'Qui possède le code produit ?',
					a: "Votre entreprise. La bounty est un contrat de type work-for-hire : la contribution est incorporée à votre repo sous votre licence, comme n'importe quelle PR merge."
				},
				{
					q: 'Comment reconnaissons-nous le talent ?',
					a: "Chaque talent a un profil public avec son historique de challenges résolus par domaine, ses badges, son streak, ses PRs mergées. Vous choisissez à qui vous confiez l'issue."
				},
				{
					q: 'Combien coûte le service ?',
					a: 'Aucun abonnement, aucun coût fixe. Skilluv prélève une commission de 15% sur la récompense au merge — visible dans la simulation avant publication.'
				}
			]
		: [
				{
					q: 'What if nobody merges the PR?',
					a: 'Escrowed credits are refunded to you on bounty expiration (30 days default, configurable). You only pay if the PR is merged by your team.'
				},
				{
					q: 'Is code quality guaranteed?',
					a: "Quality is validated by your review — you're free to reject the PR. The talent is incentivized to deliver clean work because their public profile records the acceptance rate."
				},
				{
					q: 'Who owns the produced code?',
					a: 'Your company. A bounty is a work-for-hire contract: the contribution is incorporated into your repo under your license, like any merged PR.'
				},
				{
					q: 'How do we identify the talent?',
					a: 'Each talent has a public profile with their solved challenge history by domain, badges, streak, merged PRs. You choose who to trust the issue with.'
				},
				{
					q: 'How much does the service cost?',
					a: 'No subscription, no fixed cost. Skilluv takes a 15% commission on the reward at merge — visible in the simulation before publication.'
				}
			]}
/>
