<script lang="ts">
	/**
	 * SKI-248 — one mission: the brief, what counts as done, and the terms.
	 *
...
<svelte:head>
	<title>{mission ? `${mission.title} — Skilluv` : `${i18n.t('designMissions.title')} — Skilluv`}</title>
	{#if mission}
			...
			<div class="space-y-6 lg:col-span-2">
				<section class="rounded-2xl border border-border bg-surface-elevated p-6">
					<h2 class="text-xs font-bold uppercase tracking-wider text-text-muted">
						{i18n.t('designMissions.descriptionTitle')}
					</h2>
				...
				<section class="rounded-2xl border border-border bg-surface-elevated p-6">
					<h2 class="text-xs font-bold uppercase tracking-wider text-text-muted">
						{i18n.t('designMissions.acceptanceTitle')}
					</h2>
					<p class="mt-3 whitespace-pre-wrap text-sm leading-relaxed text-text-primary">
				...
				{#if mission.nda_required}
					...
					>
						...
						{i18n.t('designMissions.ndaNotice')}
					</p>
				{/if}
			</div>
			...
			<div class="space-y-6">
				<section class="rounded-2xl border border-border bg-surface-elevated p-6">
					...
					<dl class="mt-5 space-y-3 border-t border-border pt-4 text-sm">
						<div>
							<dt class="text-xs uppercase tracking-wide text-text-muted">
								{i18n.t('designMissions.filterIpTerms')}
							</dt>
							<dd class="mt-1.5"><IpTermsBadge terms={mission.ip_terms} withHint /></dd>
						</div>
						<div class="flex items-center justify-between gap-2">
							<dt class="text-text-muted">{i18n.t('designMissions.deliverableFormats.github_pr')}</dt>
						...
						{#if mission.applications_close_at}
							<div class="inline-flex items-center gap-1.5 text-text-muted">
								...
								{i18n.t('designMissions.closesAt', {
					...
					<div class="mt-5 border-t border-border pt-4">
						{#if canApply}
							<Button variant="accent" href="/design/missions/{mission.slug}/apply" class="w-full">
								{i18n.t('designMissions.applyCta')}
							</Button>
						{:else if !auth.user}
							<Button
								variant="accent"
								href="/auth/login?redirect=/design/missions/{mission.slug}"
								class="w-full"
							>
								{i18n.t('common.nav.login')}
							</Button>
						{:else}
							<p class="text-sm text-text-muted">{i18n.t('designMissions.applyClosed')}</p>
						{/if}
					</div>
				</section>
