<script lang="ts">
	/**
	 * SKI-248 — applying to a mission.
	 *
	 * The portfolio picker draws on what the person already declared as
	 * external signals (SKI-42) rather than asking them to retype URLs they
	 * have entered once — with a free field for anything not declared. Skilluv
	 * artefacts are not offered here: the design profile already carries them,
	 * and the company reads that.
	 */
	...
	async function submit() {
		...
		try {
			...
			toast.success(i18n.t('designMissions.appliedToast'));
			await goto('/design/my/missions');
		} catch (err) {
...
<svelte:head>
	<title>{i18n.t('designMissions.applyTitle')} — Skilluv</title>
...
<div class="mx-auto max-w-2xl px-4 py-8" data-testid="design-mission-apply">
	...
	{:else if loadError}
		<div class="rounded-2xl border border-error/40 bg-error/5 p-6 text-center" role="alert">
			...
			<Button variant="ghost" size="sm" class="mt-3" href="/design/missions">
				{i18n.t('designMissions.backToList')}
			</Button>
		</div>
	{:else if mission}
		<header class="mb-6">
			<a
				href="/design/missions/{mission.slug}"
			...
			<h1 class="mt-2 text-3xl font-bold text-text-primary">
				{i18n.t('designMissions.applyTitle')}
			</h1>
			<div class="mt-3">
		...
		>
			<div>
				...
				>
					{i18n.t('designMissions.coverLetter')}
				</label>
				<textarea
					id="cover-letter"
					...
					rows="8"
					maxlength={5000}
					placeholder={i18n.t('designMissions.coverLetterPlaceholder')}
			...
			<div>
				<span class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-text-muted">
					{i18n.t('designMissions.portfolioUrls')}
				</span>
				{#if signals.length > 0}
				...
				<textarea
					...
					rows="3"
					placeholder="https://"
					aria-label={i18n.t('designMissions.portfolioUrls')}
					class="w-full rounded-xl border border-border bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:border-accent focus:outline-none"
				></textarea>
				<p class="mt-1 text-xs text-text-muted">{i18n.t('designMissions.portfolioUrlsHint')}</p>
			</div>
			...
			<div>
				<label
					...
					class="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-text-muted"
				>
					{i18n.t('designMissions.pastMissions')}
				</label>
				<textarea
					id="past-missions"
					...
					rows="4"
					maxlength={2000}
					placeholder={i18n.t('designMissions.pastMissionsPlaceholder')}
					class="w-full rounded-xl border border-border bg-surface px-3 py-2 text-sm text-text-primary placeholder:text-text-muted focus:border-accent focus:outline-none"
				></textarea>
			</div>

			<Input
				label={i18n.t('designMissions.availabilityHours')}
				type="number"
				min="1"
				max="60"
			...
			<div class="flex items-center justify-end gap-3 border-t border-border pt-5">
				<Button variant="ghost" href="/design/missions/{mission.slug}">
					{i18n.t('common.actions.cancel')}
				</Button>
				<Button variant="accent" type="submit" loading={submitting} disabled={!canSubmit}>
					{i18n.t('designMissions.applySubmit')}
				</Button>
			</div>
		</form>
	{/if}
