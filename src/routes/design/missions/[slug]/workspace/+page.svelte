<script lang="ts">
	/**
	 * M-04 / M-05 — a design mission once it is actually running.
	 *
	 ...
	 *
	 * 1. **Iterating is normal.** Two or three rounds is the expected case for
	 *    design work, not a failure, and the mission stays `in_progress` until
	 *    a round is accepted. The rounds list says so rather than letting a
	 *    second round read as trouble.
	...
	function decisionLabel(decision: string | null): string {
		if (!decision) return i18n.t('designMissionWork.awaitingDecision');
		const key = `designMissionWork.decisions.${decision}`;
	...
	async function signNda() {
		...
		try {
			...
			toast.success(i18n.t('designMissionWork.ndaSignedToast'));
			typedName = '';
			await load();
		} catch (err) {
			ndaError =
				err instanceof SkilluError && err.status === 409
					? i18n.t('designMissionWork.ndaChangedError')
	...
	async function deliver() {
		...
		try {
			...
			toast.success(i18n.t('designMissionWork.deliveredToast'));
			deliveryUrl = '';
			deliveryNotes = '';
			await load();
		} catch (err) {
	...
	async function accept() {
		...
		try {
			...
			toast.success(i18n.t('designMissionWork.acceptedToast'));
			await load();
		} catch (err) {
			toast.error(err instanceof SkilluError ? err.message : i18n.t('errors.generic'));
		} finally {
	...
	async function requestChanges() {
		...
		try {
			...
			toast.success(i18n.t('designMissionWork.requestedToast'));
			changesReason = '';
			await load();
		} catch (err) {
			toast.error(err instanceof SkilluError ? err.message : i18n.t('errors.generic'));
	...
	async function rate() {
		...
		try {
			...
			toast.success(i18n.t('designMissionWork.ratedToast'));
			ratingComment = '';
			await load();
		} catch (err) {
			toast.error(err instanceof SkilluError ? err.message : i18n.t('errors.generic'));
...
<svelte:head>
	<title>{mission?.title ?? i18n.t('designMissionWork.title')} · Skilluv</title>
...
<div class="mx-auto max-w-3xl space-y-8 px-4 py-8">
	<header class="space-y-1">
		<h1 class="text-2xl font-bold text-text">
			{mission?.title ?? i18n.t('designMissionWork.title')}
		</h1>
		<p class="text-sm text-text-muted">{i18n.t('designMissionWork.subtitle')}</p>
	</header>
	...
	{:else}
		{#if agreement}
			<section
				...
				data-testid="design-mission-nda"
			>
				<div class="flex flex-wrap items-center justify-between gap-2">
					<h2 class="flex items-center gap-2 text-sm font-bold text-text">
						<FileSignature size={16} />
						{agreement.title || i18n.t('designMissionWork.ndaTitle')}
					</h2>
					{#if signature}
						<Badge variant="success">
							{i18n.t('designMissionWork.ndaSignedOn', { date: fmtDate(signature.signed_at) })}
						</Badge>
					{/if}
				</div>
				...
				{#if !agreement.is_reviewed}
					<p class="rounded-lg border border-warning/40 bg-warning/5 px-3 py-2 text-xs text-warning">
						{i18n.t('designMissionWork.ndaUnreviewed')}
					</p>
				{/if}
				...
				{#if signature?.released_at}
					<p class="text-sm text-text-muted">
						{i18n.t('designMissionWork.ndaReleased', { date: fmtDate(signature.released_at) })}
					</p>
				{:else if !signature}
					<Input
						label={i18n.t('designMissionWork.ndaTypedName')}
						hint={i18n.t('designMissionWork.ndaTypedNameHint')}
						bind:value={typedName}
						data-testid="design-nda-name"
					/>
					{#if ndaError}
						<p class="text-sm text-error">{ndaError}</p>
					{/if}
					<Button size="sm" loading={signing} disabled={!typedName.trim()} onclick={signNda}>
						{i18n.t('designMissionWork.ndaSignCta')}
					</Button>
				{/if}
			</section>
		{/if}

		<section class="space-y-3" data-testid="design-mission-rounds">
			<h2 class="text-sm font-bold uppercase tracking-wider text-text-muted">
				{i18n.t('designMissionWork.roundsTitle')}
			</h2>
			<p class="text-xs text-text-muted">{i18n.t('designMissionWork.roundsNormal')}</p>

			{#if rounds.length === 0}
				<p class="rounded-lg border border-border bg-surface-elevated px-4 py-6 text-sm text-text-muted">
					{i18n.t('designMissionWork.roundsEmpty')}
				</p>
			{:else}
				<ol class="space-y-3">
						...
						<li class="rounded-xl border border-border bg-surface-elevated p-4">
							<div class="flex flex-wrap items-center justify-between gap-2">
								<span class="text-sm font-bold text-text">
									{i18n.t('designMissionWork.roundLabel', { n: round.round })}
								</span>
								<div class="flex items-center gap-2">
									{#if round.beyond_agreed_rounds}
										<Badge variant="warning">{i18n.t('designMissionWork.beyondAgreed')}</Badge>
									{/if}
			...
			{#if isAssignee && !waiting}
				<div class="rounded-xl border border-border bg-surface-elevated p-4 space-y-3">
					<h3 class="text-sm font-bold text-text">{i18n.t('designMissionWork.deliverTitle')}</h3>
					<Input
						label={i18n.t('designMissionWork.deliverUrl')}
						bind:value={deliveryUrl}
						placeholder="https://…"
						data-testid="design-delivery-url"
					/>
					<label class="flex flex-col gap-1">
						<span class="text-sm font-medium text-text">
							{i18n.t('designMissionWork.deliverNotes')}
						</span>
						<textarea
					...
					<Button size="sm" loading={delivering} disabled={!deliveryUrl.trim()} onclick={deliver}>
						<Upload size={15} />
						{i18n.t('designMissionWork.deliverCta')}
					</Button>
				</div>
			{/if}
			...
			{#if waiting && !isAssignee}
				<div
					...
					data-testid="design-round-decision"
				>
					<Button size="sm" loading={deciding} onclick={accept}>
						<CheckCircle2 size={15} />
						{i18n.t('designMissionWork.acceptCta')}
					</Button>
					<label class="flex flex-col gap-1">
						<span class="text-sm font-medium text-text">
							{i18n.t('designMissionWork.requestChangesReason')}
						</span>
						<textarea
						...
						<span class="text-xs text-text-muted">
							{i18n.t('designMissionWork.requestChangesHint')}
						</span>
					</label>
					<Button
						variant="secondary"
		...
		<section class="space-y-3" data-testid="design-mission-ratings">
			...
			<div class="rounded-xl border border-border bg-surface-elevated p-4 space-y-3">
				...
				<Button size="sm" loading={rating} onclick={rate}>
					{i18n.t('designMissionWork.rateCta')}
				</Button>
			</div>
		</section>

		<section class="space-y-3" data-testid="design-mission-invoices">
			<h2 class="flex items-center gap-2 text-sm font-bold uppercase tracking-wider text-text-muted">
				<Receipt size={15} />
				{i18n.t('designMissionWork.invoicesTitle')}
			</h2>

			{#if invoices.length === 0}
				<p class="rounded-lg border border-border bg-surface px-4 py-3 text-sm text-text-muted">
					{i18n.t('designMissionWork.invoicesEmpty')}
				</p>
			{:else}
				<ul class="space-y-2">
						...
						>
							<div class="space-y-0.5">
								...
								<p class="text-xs text-text-muted">
									{invoice.amount}
									{invoice.currency} · {i18n.t('designMissionWork.invoiceCommission')}
							...
							<div class="flex items-center gap-2">
								...
								{#if invoice.status !== 'paid'}
									<Button size="sm" variant="ghost" onclick={() => pay(invoice.id)}>
										{i18n.t('designMissionWork.payCta')}
									</Button>
								{/if}
							</div>
						</li>
