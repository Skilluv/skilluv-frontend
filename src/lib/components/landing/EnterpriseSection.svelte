...
<section class="py-16 sm:py-24 lg:py-32">
	<div class="mx-auto max-w-7xl px-4">
		<div use:scrollReveal class="mb-10 sm:mb-16">
			...
			>
				...
				<span class="text-primary"
					>{i18n.locale === 'fr' ? 'Bienvenue dans la preuve.' : 'Welcome to proof.'}</span
				>
			</h2>
		...
		<div
			use:scrollReveal
			class="rounded-2xl border-2 border-cat-understand bg-surface-understand overflow-hidden"
		>
			<!-- Toolbar -->
			<div class="flex items-center gap-3 border-b border-border px-5 py-3 overflow-x-auto">
				<span class="text-xs font-semibold shrink-0"
					>{i18n.locale === 'fr' ? 'Recherche' : 'Search'}</span
				>
				<div class="flex gap-2 shrink-0">
					<span
						class="rounded-md {codeStyle.bgSoft} px-2.5 py-1 text-[11px] font-medium {codeStyle.text}"
						>Code</span
					>
					<span
						class="rounded-md bg-surface-overlay px-2.5 py-1 text-[11px] font-medium text-text-muted"
						>Rust</span
					>
					<span
						class="rounded-md bg-surface-overlay px-2.5 py-1 text-[11px] font-medium text-text-muted"
						>{i18n.locale === 'fr' ? 'Artisan+' : 'Artisan+'}</span
					>
				</div>
				<span class="ml-auto text-[11px] text-text-muted shrink-0"
					>{i18n.locale === 'fr' ? '23 résultats' : '23 results'}</span
				>
			</div>
			...
			<div class="divide-y divide-border">
				{#each demoTalents as talent}
					<div class="flex items-center gap-4 px-5 py-4">
						<!-- Avatar -->
						<div
							class="shrink-0 h-10 w-10 rounded-full bg-surface-overlay flex items-center justify-center text-sm font-bold text-primary"
						>
							{talent.name[0]}
						</div>
						...
						<div class="hidden sm:flex gap-1.5 shrink-0">
							{#each talent.skills as skill}
								<span
									class="rounded-md bg-surface-overlay px-2 py-0.5 text-[10px] font-medium text-text-muted"
									>{skill}</span
								>
							{/each}
						</div>
						...
						<!-- Action -->
						<button
							class="shrink-0 rounded-lg border border-border px-3 py-1.5 text-xs font-medium text-text-muted transition-colors duration-200 hover:border-primary hover:text-primary"
						>
			...
			<div class="border-t border-border px-5 py-3 flex items-center justify-between">
				...
				<span class="text-[11px] text-text-muted"
					>{i18n.locale === 'fr' ? 'Page 1/3' : 'Page 1/3'}</span
				>
			</div>
		</div>
		...
		<div use:scrollReveal class="mt-8 grid sm:grid-cols-2 gap-4">
			...
			<div class="rounded-2xl border-2 border-border bg-surface-alt p-5">
				<p class="mb-3 text-[10px] uppercase tracking-widest text-text-muted">
					{i18n.t('commonExtra.exampleLabel')}
				</p>
				<p
					class="text-xs text-text-muted uppercase tracking-wider mb-4 line-through decoration-error/50"
				>
					{i18n.locale === 'fr' ? 'Recrutement classique' : 'Traditional hiring'}
				</p>
				<ul class="space-y-2.5 text-sm text-text-muted">
					<li class="flex items-start gap-2">
						...
						<span
							>{i18n.locale === 'fr'
								? 'CV déclaratif, non vérifiable'
								: 'Self-declared resume, unverifiable'}</span
						>
					</li>
					<li class="flex items-start gap-2">
						<span class="shrink-0 text-error mt-0.5">×</span>
						<span
							>{i18n.locale === 'fr'
								? 'Tests techniques génériques'
								: 'Generic technical tests'}</span
						>
					</li>
					<li class="flex items-start gap-2">
						<span class="shrink-0 text-error mt-0.5">×</span>
						<span
							>{i18n.locale === 'fr'
								? "Aucune visibilité sur l'engagement"
								: 'No visibility on engagement'}</span
						>
					</li>
				</ul>
			</div>
			...
			<div class="rounded-2xl border-2 border-cat-meta bg-surface-meta p-5">
				...
				<ul class="space-y-2.5 text-sm text-text-muted">
					<li class="flex items-start gap-2">
						...
						<span
							>{i18n.locale === 'fr'
								? 'Compétences prouvées par des challenges réels'
								: 'Skills proven through real challenges'}</span
						>
					</li>
					<li class="flex items-start gap-2">
						<span class="shrink-0 text-success mt-0.5">+</span>
						<span
							>{i18n.locale === 'fr'
								? 'Niveau, streak, progression visibles'
								: 'Level, streak, progression visible'}</span
						>
					</li>
					<li class="flex items-start gap-2">
						<span class="shrink-0 text-success mt-0.5">+</span>
						<span
							>{i18n.locale === 'fr'
								? 'Contact direct, respectueux du talent'
								: 'Direct contact, respectful of talent'}</span
						>
					</li>
				</ul>
			</div>
