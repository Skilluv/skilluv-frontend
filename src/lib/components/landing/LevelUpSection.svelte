	...
	<div class="mx-auto max-w-7xl px-4">
		...
		<div use:scrollReveal class="mt-8 grid sm:grid-cols-2 gap-4">
			...
			<div class="rounded-2xl border-2 border-border bg-surface-alt p-5">
				<p class="text-[10px] text-text-muted uppercase tracking-wider mb-3">
					{i18n.locale === 'fr' ? 'Jour 1' : 'Day 1'}
				</p>
				<div class="flex items-center gap-3 mb-3">
					<div
						class="h-9 w-9 rounded-full bg-surface-overlay flex items-center justify-center text-xs font-bold text-text-muted"
					>
						?
					</div>
					<div>
						<p class="text-sm font-semibold text-text-muted">
							{i18n.locale === 'fr' ? 'Nouveau talent' : 'New talent'}
						</p>
						<p class="text-xs text-text-muted">{i18n.t('common.titles.apprenti')}</p>
					</div>
				</div>
				<div class="flex gap-2">
					<span class="rounded-md bg-surface-overlay px-2 py-0.5 text-[10px] text-text-muted/50"
						>{i18n.locale === 'fr' ? 'Aucun badge' : 'No badges'}</span
					>
					<span class="rounded-md bg-surface-overlay px-2 py-0.5 text-[10px] text-text-muted/50"
						>{i18n.locale === 'fr' ? 'Aucun challenge' : 'No challenges'}</span
					>
				</div>
			</div>
			...
			<div class="rounded-2xl border-2 border-cat-craft bg-surface-craft p-5">
				<p class="text-[10px] text-amber-400 uppercase tracking-wider mb-1">
					{i18n.locale === 'fr' ? '6 mois plus tard' : '6 months later'}
				</p>
				...
				<div class="flex items-center gap-3 mb-3">
					<div
						class="h-9 w-9 rounded-full bg-amber-500/15 flex items-center justify-center text-xs font-bold text-amber-400"
					>
						A
					</div>
					<div>
				...
				<div class="flex gap-2">
					{#each ['Rust', 'Go', 'TypeScript'] as skill}
						<span
							class="rounded-md {codeStyle.bgSoft} px-2 py-0.5 text-[10px] font-medium {codeStyle.text}"
							>{skill}</span
						>
					{/each}
				</div>
