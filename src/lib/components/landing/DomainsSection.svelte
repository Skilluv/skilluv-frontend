	...
	<div class="mx-auto max-w-7xl px-4">
		...
		<div class="grid lg:grid-cols-2 gap-4">
			{#each domains as domain}
				...
				{@const catMap: Partial<Record<SkillDomain, string>> = {
					...
					ai: 'bg-surface-create border-cat-create',
					ops: 'bg-surface-operate border-cat-operate',
					soft_skills: 'bg-surface-meta border-cat-meta'
				}}
				<a
					href="/challenges?domain={domain.key}"
					use:scrollReveal
					class="group rounded-2xl border-2 {catMap[domain.key] ?? 'bg-surface-elevated border-border'} overflow-hidden transition-colors duration-200 {ds.hoverBorder}"
				>
					<!-- Domain header bar -->
