<script lang="ts">
	...
	function shareX() {
		window.open(
			`https://x.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(fullUrl)}`,
			'_blank',
...
<div class="relative">
	...
	{#if showMenu}
		...
		<div
			class="absolute right-0 top-full z-50 mt-2 w-48 rounded-xl border border-border bg-surface-elevated p-2 shadow-lg animate-[slide-up_150ms_ease-out]"
		>
			<button
				class="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-sm text-text-muted hover:bg-surface-overlay hover:text-text-primary"
				onclick={shareX}
			>
				X / Twitter
			</button>
